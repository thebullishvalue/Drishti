...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Reveals the fundamental predictive truth (tattva) of every feature
using the most advanced 2026 mathematical & physics toolkit:
• SHAP Game Theory (exact Shapley values)
• Hybrid Graph Topology (PageRank + Eigenvector on dCor network)
• Brownian Distance Correlation + HSIC (kernel independence)
• Partial Correlation (conditional dependence)
• Granger + simple Transfer-Entropy style causality
• Mutual Information + RF + VIF

v5.0-GodMode — Backend completely rebuilt with frontier techniques.
UI/UX 100% untouched. Processing only on explicit "RUN ANALYSIS".
"""
...
# Constants
# ────────────────────────────────────────────────────────────────────────────
VERSION = "v5.0-GodMode"
PRODUCT_NAME = "Tattva"
COMPANY = "Hemrek Capital"

st.set_page_config(page_title="TATTVA | Essence Matrix", page_icon="✦", layout="wide", initial_sidebar_state="collapsed")

# ────────────────────────────────────────────────────────────────────────────
# Premium CSS (Nirnay Design System) — EXACTLY UNCHANGED
# ────────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
    ...
    * { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; }
    ...
    [data-testid="collapsedControl"] { display: flex !important; visibility: visible !important; opacity: 1 !important; background-color: var(--secondary-background-color) !important; border: 2px solid var(--primary-color) !important; border-radius: 8px !important; padding: 10px !important; margin: 12px !important; box-shadow: 0 0 15px rgba(var(--primary-rgb), 0.4) !important; z-index: 999999 !important; position: fixed !important; top: 14px !important; left: 14px !important; width: 40px !important; height: 40px !important; align-items: center !important; justify-content: center !important; }
    [data-testid="collapsedControl"]:hover { background-color: rgba(var(--primary-rgb), 0.2) !important; box-shadow: 0 0 20px rgba(var(--primary-rgb), 0.6) !important; transform: scale(1.05); }
    [data-testid="collapsedControl"] svg { stroke: var(--primary-color) !important; width: 20px !important; height: 20px !important; }
    
    .premium-header { background: var(--secondary-background-color); padding: 1.25rem 2rem; border-radius: 16px; margin-bottom: 1.5rem; box-shadow: 0 0 20px rgba(var(--primary-rgb), 0.1); border: 1px solid var(--border-color); position: relative; overflow: hidden; margin-top: 1rem; }
    ...
    .metric-card { background-color: var(--bg-card); padding: 1.25rem; border-radius: 12px; border: 1px solid var(--border-color); box-shadow: 0 0 15px rgba(var(--primary-rgb), 0.08); margin-bottom: 0.5rem; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative; overflow: hidden; min-height: 160px; display: flex; flex-direction: column; justify-content: center; }
...
# ────────────────────────────────────────────────────────────────────────────
# FRONTIER PHYSICS & MATH UTILITIES (v5.0 GodMode)
# ────────────────────────────────────────────────────────────────────────────
...
def hsic(x, y):
    """Hilbert-Schmidt Independence Criterion (kernel-based) — frontier non-linear independence test"""
    ...
    sigma_x = np.median(dx[dx > 0]) or 1.0
    sigma_y = np.median(dy[dy > 0]) or 1.0
    K = np.exp(-dx**2 / (2 * sigma_x**2))
    L = np.exp(-dy**2 / (2 * sigma_y**2))
    H = np.eye(n) - 1.0 / n
    return float(np.trace(K @ H @ L @ H) / n**2)


def partial_correlation_target(X, y):
    """Partial correlation of each feature with target controlling for all others (precision matrix)"""
    ...
    try:
        ...
        return np.abs(partial[0, 1:])  # absolute partial corr vs target
    except:
...
# ────────────────────────────────────────────────────────────────────────────
# TATTVA ENGINE — v5.0 GODMODE (frontier math/physics)
# ────────────────────────────────────────────────────────────────────────────
...
class TattvaEngine:
    ...
    def analyze(self, progress_callback=None):
        ...
        if SKLEARN_AVAILABLE:
            if SHAP_AVAILABLE:
                try:
                    ...
                    adv_imp = np.abs(shap_vals).mean(axis=0)
                except:
                    pass
            if np.all(adv_imp == 0):
                try:
        ...
        # Frontier Topology + Partial Correlation
        if len(self.features) > 1:
            ...
            topo_scores = (eigen_c + pr_c) / 2
        else:
            topo_scores = np.ones(len(self.features))

        partial_scores = partial_correlation_target(self.X_scaled, self.y_scaled)

        if progress_callback: progress_callback(75)
        # HSIC (kernel independence)
        hsic_scores = np.array([hsic(self.X_scaled[:, i], self.y_scaled) for i in range(len(self.features))])
        ...
        for i, feat in enumerate(self.features):
            ...
            self.results.append({
                ...
                'Topological_Centrality': float(topo_scores[i]),
                'Partial_Corr': float(partial_scores[i]),
                'HSIC': float(hsic_scores[i]),
    ...
    def _calculate_composite_score(self):
        ...
        # GodMode weights (sum=1.0) — frontier techniques now dominate
        score = (
            norm('Abs_Pearson')        * 0.03 +
            norm('Spearman')           * 0.03 +
            norm('Distance_Corr')      * 0.12 +
            norm('Mutual_Info')        * 0.10 +
            norm('RF_Importance')      * 0.05 +
            norm('Advanced_Importance')* 0.20 +   # SHAP Game Theory
            norm('Topological_Centrality') * 0.12 +
            norm('Partial_Corr')       * 0.10 +   # Conditional dependence
            norm('HSIC')               * 0.13 +   # Kernel independence
    ...
    def get_insights(self):
        ...
        df['NonLinear_Bias'] = (norm(df['Distance_Corr']) + norm(df['Mutual_Info']) + norm(df['HSIC'])) / 3 - norm(df['Abs_Pearson'])
...
def render_landing_page():
    ...
    with col1:
        st.markdown("""<div class='metric-card purple' style='min-height: 280px; justify-content: flex-start;'>
            <h3 style='color: var(--purple); margin-bottom: 0.5rem;'>🌌 Information Theory</h3>
            <p style='color: var(--text-muted); font-size: 0.9rem; line-height: 1.6;'>Measures the actual "bits" of predictive truth each feature carries.</p>
        </div>""", unsafe_allow_html=True)
    with col2:
        st.markdown("""<div class='metric-card info' style='min-height: 280px; justify-content: flex-start;'>
            <h3 style='color: var(--info-cyan); margin-bottom: 0.5rem;'>🧬 Energy + Kernel Statistics</h3>
            <p style='color: var(--text-muted); font-size: 0.9rem; line-height: 1.6;'>Brownian dCor + HSIC — true independence detection.</p>
        </div>""", unsafe_allow_html=True)
    with col3:
        st.markdown("""<div class='metric-card primary' style='min-height: 280px; justify-content: flex-start;'>
            <h3 style='color: var(--primary-color); margin-bottom: 0.5rem;'>🕸️ Network + Conditional Topology</h3>
            <p style='color: var(--text-muted); font-size: 0.9rem; line-height: 1.6;'>Hybrid centrality + Partial Correlation.</p>
        </div>""", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("### 🎯 Signal Interpretation")
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown("""<div style='background: rgba(16,185,129,0.1); border:1px solid var(--success-green); border-radius:12px; padding:1.25rem;'><h4 style='color:var(--success-green);'>🟢 High Essence Score</h4><p style='color:var(--text-muted); font-size:0.85rem;'>Score > 75</p><p style='color:var(--text-secondary); font-size:0.85rem; margin-top:0.5rem;'>Core predictive truth — must keep.</p></div>""", unsafe_allow_html=True)
    with c2: st.markdown("""<div style='background: rgba(6,182,212,0.1); border:1px solid var(--info-cyan); border-radius:12px; padding:1.25rem;'><h4 style='color:var(--info-cyan);'>🔵 Hidden Non-Linear Value</h4><p style='color:var(--text-muted); font-size:0.85rem;'>Low Pearson, High dCor/HSIC</p><p style='color:var(--text-secondary); font-size:0.85rem; margin-top:0.5rem;'>Powerful in complex models.</p></div>""", unsafe_allow_html=True)
    with c3: st.markdown("""<div style='background: rgba(239,68,68,0.1); border:1px solid var(--danger-red); border-radius:12px; padding:1.25rem;'><h4 style='color:var(--danger-red);'>🔴 Redundant / Toxic</h4><p style='color:var(--text-muted); font-size:0.85rem;'>VIF > 10</p><p style='color:var(--text-secondary); font-size:0.85rem; margin-top:0.5rem;'>High collinearity — drop.</p></div>""", unsafe_allow_html=True)


# ────────────────────────────────────────────────────────────────────────────
# MAIN APPLICATION — Processing ONLY on explicit button click
# ────────────────────────────────────────────────────────────────────────────

def main():
    with st.sidebar:
        st.markdown("""<div style="text-align: center; padding: 1rem 0; margin-bottom: 1rem;"><div style="font-size: 1.75rem; font-weight: 800; color: #FFC300;">TATTVA</div><div style="color: #888888; font-size: 0.75rem; margin-top: 0.25rem;">तत्त्व | Essence Matrix</div></div>""", unsafe_allow_html=True)
    ...
    if df is None:
        st.markdown("""<div class="premium-header"><h1>TATTVA : Essence Matrix</h1><div class="tagline">Revealing the fundamental predictive truth of your features.</div></div>""", unsafe_allow_html=True)
        render_landing_page()
        render_footer()
        return
    ...
    with st.sidebar:
        ...
        date_col = st.selectbox("Time Axis", ["None"] + all_cols, index=all_cols.index(date_candidates[0])+1 if date_candidates else 0)

        st.markdown('<br>', unsafe_allow_html=True)
        run_btn = st.button("🚀 RUN ANALYSIS", type="primary")

        st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
        st.markdown(f"""<div class='info-box'><p style='font-size: 0.8rem; margin: 0; color: var(--text-muted); line-height: 1.5;'><strong>Version:</strong> {VERSION}<br><strong>Engine:</strong> SHAP • HSIC • Partial Corr • Hybrid Topology • Granger</p></div>""", unsafe_allow_html=True)

    # Config hash for auto-reset on column change
    config_hash = f"{target_col}_{'-'.join(sorted(feature_cols))}_{date_col}"
    if 'tattva_config' not in st.session_state or st.session_state.tattva_config != config_hash:
        st.session_state.is_analyzed = False
        st.session_state.tattva_config = config_hash

    if run_btn:
        st.session_state.is_analyzed = True

    if not st.session_state.get('is_analyzed', False):
        st.markdown("""<div class="premium-header"><h1>TATTVA : Essence Matrix</h1><div class="tagline">Revealing the fundamental predictive truth of your features.</div></div>""", unsafe_allow_html=True)
        render_landing_page()
        render_footer()
        return
    ...
    if len(data) < 30:
        st.error("Not enough clean rows (need ≥ 30).")
        return
    ...
    if 'tattva_cache' not in st.session_state or st.session_state.tattva_cache != cache_key:
        with st.spinner("Extracting feature essence — frontier physics & game theory..."):
            progress_bar = st.progress(0)
            def update_progress(p):
                progress_bar.progress(p)
            engine = TattvaEngine(data, target_col, feature_cols, date_col if date_col != "None" else None)
            engine.analyze(update_progress)
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["**🎯 Essence Ranking**","**🧬 Correlations**","**🌌 Advanced Dependence**","**🕸️ Topology & VIF**","**📋 Full Table**"])

    with tab1:
        st.markdown("##### Composite Essence Score")
        fig_bar = px.bar(res_df, x='Composite_Score', y='Feature', orientation='h', color='Composite_Score', color_continuous_scale=['#ef4444','#f59e0b','#10b981','#FFC300'], hover_data=['Pearson','Distance_Corr','VIF'])
        ...
        with col1:
            if insights["redundant_features"]:
                st.markdown(f"""<div class="guide-box danger"><strong>⚠️ High Collinearity</strong><br>Features: {", ".join(insights["redundant_features"])}<br>Pick highest scoring one.</div>""", unsafe_allow_html=True)
            else:
                st.markdown("""<div class="guide-box success"><strong>✓ Clean structure</strong><br>No severe multicollinearity.</div>""", unsafe_allow_html=True)
        with col2:
            if insights["hidden_nonlinear"]:
                st.markdown(f"""<div class="guide-box info"><strong>🌌 Non-linear goldmine</strong><br>Features: {", ".join(insights["hidden_nonlinear"])}<br>Powerful in trees/NNs.</div>""", unsafe_allow_html=True)

    with tab2:
        st.markdown("##### Pearson Correlation Heatmap")
        fig_heat = go.Figure(go.Heatmap(z=engine.corr_matrix.values, x=engine.corr_matrix.columns, y=engine.corr_matrix.columns, colorscale='RdBu', zmin=-1, zmax=1, text=np.round(engine.corr_matrix.values, 2), texttemplate="%{text}", hoverinfo="x+y+z"))
    ...
    with tab3:
        ...
        fig_scatter = px.scatter(res_df, x='Abs_Pearson', y='Distance_Corr', text='Feature', size='Mutual_Info', color='Composite_Score', color_continuous_scale='Viridis', hover_data=['Spearman'])
    ...
    with tab4:
        ...
        fig_vif = go.Figure(go.Bar(x=vif_df['Feature'], y=vif_df['VIF'], marker_color=colors, text=np.round(vif_df['VIF'],1), textposition='auto'))
