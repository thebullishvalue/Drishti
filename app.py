...
using advanced mathematical & physics-based toolkit.

v5.0-GodMode — SHAP • HSIC • Partial Corr • Hybrid Topology • Granger
All values displayed with 2 decimal places.
Includes actionable "Recommended Feature Utilization Summary".
"""
...
# ────────────────────────────────────────────────────────────────────────────
# Math / Physics Utilities (unchanged from previous GodMode version)
# ────────────────────────────────────────────────────────────────────────────
...
def hsic(x, y):
    ...
    sigma_x = np.median(dx[dx > 0]) if np.any(dx > 0) else 1.0
    sigma_y = np.median(dy[dy > 0]) if np.any(dy > 0) else 1.0
...
class TattvaEngine:
    ...
    def analyze(self, progress_callback=None):
        ...
        for i, feat in enumerate(self.features):
            ...
            self.results.append({
                ...
                'Abs_Pearson': round(abs(pearson), 2),
                'Spearman': round(spearman, 2),
                'Distance_Corr': round(dcor, 2),
                'Mutual_Info': round(mi_scores[i], 2),
                'RF_Importance': round(rf_importance[i], 2),
                'Advanced_Importance': round(adv_imp[i], 2),
                'VIF': round(self.vif_data.get(feat, np.nan), 2) if not np.isnan(self.vif_data.get(feat, np.nan)) else np.nan,
                'Topological_Centrality': round(float(topo_scores[i]), 2),
                'Partial_Corr': round(float(partial_scores[i]), 2),
                'HSIC': round(float(hsic_scores[i]), 2),
                'Granger_Score': round(granger, 2)
            })
    ...
    def _calculate_composite_score(self):
        ...
        df['Composite_Score'] = (score * vif_penalty).round(2)
...
def render_landing_page():
    ...
    with c1:
        st.markdown("""<div style='background: rgba(16,185,129,0.1); border:1px solid var(--success-green); border-radius:12px; padding:1.25rem;'>
            <h4 style='color:var(--success-green);'>🟢 High Essence Score</h4>
            <p style='color:var(--text-muted); font-size:0.85rem;'>Score > 75</p>
            <p style='color:var(--text-secondary); font-size:0.85rem; margin-top:0.5rem;'>Core predictive truth — must keep.</p>
        </div>""", unsafe_allow_html=True)
    with c2:
        st.markdown("""<div style='background: rgba(6,182,212,0.1); border:1px solid var(--info-cyan); border-radius:12px; padding:1.25rem;'>
            <h4 style='color:var(--info-cyan);'>🔵 Hidden Non-Linear Value</h4>
            <p style='color:var(--text-muted); font-size:0.85rem;'>Low Pearson, High dCor/HSIC</p>
            <p style='color:var(--text-secondary); font-size:0.85rem; margin-top:0.5rem;'>Powerful in complex models.</p>
        </div>""", unsafe_allow_html=True)
    with c3:
        st.markdown("""<div style='background: rgba(239,68,68,0.1); border:1px solid var(--danger-red); border-radius:12px; padding:1.25rem;'>
            <h4 style='color:var(--danger-red);'>🔴 Redundant / Toxic</h4>
            <p style='color:var(--text-muted); font-size:0.85rem;'>VIF > 10</p>
            <p style='color:var(--text-secondary); font-size:0.85rem; margin-top:0.5rem;'>High collinearity — drop.</p>
        </div>""", unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(f"""<div class='info-box'><p style='font-size: 0.8rem; margin: 0; color: var(--text-muted); line-height: 1.5;'>
            ...
            <strong>Engine:</strong> SHAP • HSIC • Partial Corr • Hybrid Topology • Granger
    ...
    # ────────────────────────────────────────────────────────────────────────
    # Top Metrics + New Summary
    # ────────────────────────────────────────────────────────────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(f'<div class="metric-card primary"><h4>Primary Essence</h4><h2>{insights["top_feature"]}</h2><div class="sub-metric">Score: {insights["top_score"]:.2f}/100</div></div>', unsafe_allow_html=True)
    with c2:
        n_hidden = len(insights["hidden_nonlinear"])
        txt = ", ".join(insights["hidden_nonlinear"])[:40] + "..." if n_hidden and len(", ".join(insights["hidden_nonlinear"])) > 40 else ", ".join(insights["hidden_nonlinear"]) if n_hidden else "None"
    ...
    with c4:
        ...
        st.markdown(f'<div class="metric-card {col}"><h4>Information Gain</h4><h2>{avg_mi:.2f}</h2><div class="sub-metric">Average MI</div></div>', unsafe_allow_html=True)

    # ──── Recommended Feature Utilization Summary ────
    st.markdown("### Recommended Feature Utilization Summary")
    st.markdown("<div style='background: rgba(255,195,0,0.08); border: 1px solid var(--primary-color); border-radius: 12px; padding: 1.25rem; margin-bottom: 1.5rem;'>", unsafe_allow_html=True)

    top_features = res_df.head(8)['Feature'].tolist()
    must_keep = res_df[res_df['Composite_Score'] > 70]['Feature'].tolist()
    strong_nonlinear = insights['hidden_nonlinear']
    drop_candidates = insights['redundant_features']

    colA, colB = st.columns([3, 2])

    with colA:
        if must_keep:
            st.markdown("**Strongly recommended to keep** (Composite Score > 70):")
            st.markdown(" • " + "  \n • ".join(must_keep[:6]))
        else:
            st.markdown("**No extremely dominant features** (>70) — review top 5–8 based on domain.")

        if strong_nonlinear:
            st.markdown("\n**Non-linear hidden gems** (use in tree/NN models):")
            st.markdown(" • " + "  \n • ".join(strong_nonlinear))

    with colB:
        if drop_candidates:
            st.markdown("**Likely candidates to drop** (high VIF / redundancy):")
            st.markdown(" • " + "  \n • ".join(drop_candidates[:5]))
            st.markdown("*(Keep the highest-scoring one if domain-critical)*")
        else:
            st.markdown("**No strong redundancy detected** — features appear relatively independent.")
    ...
    with tab1:
        ...
        fig_bar = px.bar(
            ...
            hover_data={k: ':.2f' for k in ['Pearson','Spearman','Distance_Corr','VIF']}
        )
    ...
    with tab2:
        ...
        fig_heat = go.Figure(go.Heatmap(
            z=np.round(engine.corr_matrix.values, 2),
